-- Reliable AI-job mutations without relying on client-side table write policies.
...
begin
  ...
  if requested_kind not in ('document_extraction', 'quote_generation', 'reply_draft') then
    raise exception 'Invalid AI job type';
  end if;
  insert into public.ai_runs (company_id, initiated_by, kind, status, prompt_version, started_at)
  values (target_company_id, auth.uid(), requested_kind, 'running', requested_prompt_version, now())
...
as $$
begin
  if auth.uid() is null or not exists (select 1 from public.ai_runs r where r.id = target_run_id and public.is_company_member(r.company_id)) then
    raise exception 'Not allowed for this AI job';
  end if;
  update public.ai_runs set status = new_status, quote_id = coalesce(target_quote_id, quote_id), model = coalesce(provider_model, model), input_tokens = coalesce(provider_input_tokens, input_tokens), output_tokens = coalesce(provider_output_tokens, output_tokens), error_code = failure_code, finished_at = now()
