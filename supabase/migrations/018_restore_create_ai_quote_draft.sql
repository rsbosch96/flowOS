-- RSTECH-TASK-008: restore the atomic quote-draft RPC in projects where
-- the Gateway migrations were applied out of order. No existing records or
-- tables are changed; this function is the sole missing dependency.
...
begin
  ...
  if length(trim(customer_name)) < 2 or length(trim(draft_title)) < 3 then
  ...
  loop
    ...
    if coalesce(length(trim(item->>'description')), 0) < 2 then
    ...
    insert into public.quote_items (
      ...
      new_quote_id, item_position, trim(item->>'description'), item_quantity,
      item_unit, item_price_cents, item_vat_rate, item_total_cents
    );
  end loop;
