-- Repair an incomplete application of migration 023 without replaying that migration.
-- Apply after 024_invoice_integrity_and_numbering.sql and 025_fix_pgcrypto_schema_qualification.sql.
-- Existing valid RPCs are intentionally left unchanged; only missing signatures are created.
...
-- The reset is a no-op when migration 023 already revoked all legacy raw links.
-- It deliberately targets only raw-token records and never revokes a valid hashed link.
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'quote_email_deliveries'
      and policyname = 'tenant quote email deliveries read'
  ) then
...
-- Use the qualified pgcrypto schema if the helper was skipped when 023 stopped early.
do $$
begin
  if to_regprocedure('public.hash_public_quote_token(text)') is null then
    execute $function$
      create function public.hash_public_quote_token(raw_token text)
      returns text
      language sql
      immutable
      strict
...
do $$
begin
  if to_regprocedure('public.rotate_public_quote_token(uuid,uuid,integer)') is null then
    execute $function$
      create function public.rotate_public_quote_token(
...
do $$
begin
  if to_regprocedure('public.revoke_public_quote_token(uuid,uuid)') is null then
    execute $function$
      create function public.revoke_public_quote_token(target_quote_id uuid, target_company_id uuid)
      returns boolean
      language plpgsql
      security definer
      set search_path = public
...
do $$
begin
  if to_regprocedure('public.get_public_quote(text)') is null then
    execute $function$
      create function public.get_public_quote(raw_token text)
      returns jsonb
      language sql
      security definer
      set search_path = public
...
do $$
begin
  if to_regprocedure('public.customer_decide_quote(text,public.quote_status,text)') is null then
    execute $function$
      create function public.customer_decide_quote(raw_token text, decision public.quote_status, comment text default null)
      returns boolean
      language plpgsql
      security definer
      set search_path = public
...
do $$
begin
  if to_regprocedure('public.customer_question_quote(text,text)') is null then
    execute $function$
      create function public.customer_question_quote(raw_token text, question text)
      returns boolean
      language plpgsql
      security definer
      set search_path = public
...
do $$
begin
  if to_regprocedure('public.reserve_quote_email_delivery(uuid,uuid,text,text,uuid)') is null then
    execute $function$
      create function public.reserve_quote_email_delivery(
        target_company_id uuid,
        target_quote_id uuid,
...
do $$
begin
  if to_regprocedure('public.complete_quote_email_delivery(uuid,uuid,text)') is null then
    execute $function$
      create function public.complete_quote_email_delivery(target_delivery_id uuid, target_company_id uuid, provider_message text)
      returns boolean
      language plpgsql
      security definer
      set search_path = public
...
do $$
begin
  if to_regprocedure('public.fail_quote_email_delivery(uuid,uuid,text)') is null then
    execute $function$
      create function public.fail_quote_email_delivery(target_delivery_id uuid, target_company_id uuid, safe_error_code text)
      returns boolean
      language plpgsql
      security definer
      set search_path = public
...
-- Disable only the legacy raw-token RPC signatures when they are still present.
do $$
begin
  if to_regprocedure('public.publish_quote_for_customer(uuid,integer)') is not null then
    revoke all on function public.publish_quote_for_customer(uuid, integer) from public, anon, authenticated;
  end if;
  if to_regprocedure('public.get_public_quote(uuid)') is not null then
    revoke all on function public.get_public_quote(uuid) from public, anon, authenticated;
  end if;
  if to_regprocedure('public.customer_decide_quote(uuid,public.quote_status,text)') is not null then
    revoke all on function public.customer_decide_quote(uuid, public.quote_status, text) from public, anon, authenticated;
  end if;
  if to_regprocedure('public.customer_question_quote(uuid,text)') is not null then
