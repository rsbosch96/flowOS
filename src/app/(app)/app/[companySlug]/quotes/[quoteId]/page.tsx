...
export default async function QuoteDetailPage({ params }: { params: Promise<{ companySlug: string; quoteId: string }> }) {
  ...
  const planningEnabled = await hasCompanyModule(supabase, company.id, planningModule);
  ...
  return <div className="max-w-4xl space-y-6">
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="text-right"><p className="rounded-full bg-slate-100 px-3 py-1 text-sm">{quoteStatusLabel(quote.status)}</p>{quote.status === "draft" && <div className="mt-3"><ApproveQuoteButton companyId={company.id} quoteId={quote.id} /></div>}{quote.status === "approved" && <div className="mt-3 flex justify-end gap-2"><PublishQuoteButton companyId={company.id} quoteId={quote.id} /><EmailQuoteButton companyId={company.id} quoteId={quote.id} /></div>}{quote.status === "sent" && <div className="mt-3 flex justify-end gap-2"><EmailQuoteButton companyId={company.id} quoteId={quote.id} /><RemindQuoteButton companyId={company.id} quoteId={quote.id} /></div>}{quote.status === "accepted" && <div className="mt-3"><CreateInvoiceButton companyId={company.id} quoteId={quote.id} companySlug={companySlug} />{planningEnabled && <AddQuoteToPlanningButton companyId={company.id} companySlug={companySlug} quoteId={quote.id} quoteNumber={quote.quote_number} quoteTitle={quote.title} />}</div>}</div>
      </div>
