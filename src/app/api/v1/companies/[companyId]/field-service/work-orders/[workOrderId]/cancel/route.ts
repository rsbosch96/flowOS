...
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string; workOrderId: string }> }) {
  ...
  return withApiRequest(request, { route: "/api/v1/companies/:companyId/field-service/work-orders/:workOrderId/cancel", companyId }, async (requestId) => {
    ...
    return runFieldServiceTransitionRpc(requestId, "/api/v1/companies/:companyId/field-service/work-orders/:workOrderId/cancel", companyId, workOrderId, "cancel_field_service_work_order");
  });
}
