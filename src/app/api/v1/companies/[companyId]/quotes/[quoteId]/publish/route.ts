...
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string; quoteId: string }> }) {
  ...
  const rateLimitError = await enforceRateLimit({ policy: "quote_publish_or_token", subjectParts: [quote.company_id, quote.id], requestId, route: "/api/v1/companies/:companyId/quotes/:quoteId/publish", companyId: quote.company_id, actorId: user.id });
