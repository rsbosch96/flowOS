...
const schema = z.object({ name: z.string().trim().min(2).max(160), description: z.string().trim().max(2000).nullable(), sku: z.string().trim().max(60).nullable(), unit: z.string().trim().min(1).max(30), priceCents: z.number().int().nonnegative(), vatRate: z.number().min(0).max(100) });
export async function POST(request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  const { error } = await supabase.from("product_catalog_items").insert({ company_id: companyId, name: body.data.name, description: body.data.description || null, sku: body.data.sku || null, unit: body.data.unit, default_unit_price_cents: body.data.priceCents, default_vat_rate: body.data.vatRate });
