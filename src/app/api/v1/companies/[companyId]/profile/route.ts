...
const profileSchema = z.object({
  ...
  kvkNumber: optionalText(32), vatNumber: optionalText(32), street: optionalText(160), postalCode: optionalText(24), city: optionalText(120), country: optionalText(100), phone: optionalText(40),
...
export async function GET(_request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  const { data: company, error } = await access.supabase.from("companies").select("id,name,kvk_number,vat_number,address,phone,email,website").eq("id", companyId).maybeSingle();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ companyId: string }> }) {
  ...
  const { error: companyError } = await access.supabase.from("companies").update({ name: input.data.name, kvk_number: emptyToNull(input.data.kvkNumber), vat_number: emptyToNull(input.data.vatNumber), address, phone: emptyToNull(input.data.phone), email: emptyToNull(input.data.email), website: emptyToNull(input.data.website), updated_at: new Date().toISOString() }).eq("id", companyId);
